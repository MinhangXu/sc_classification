...
# Resume/extend Plan 0 standard DR caches for FactoSig promax branch in place.
...
usage() {
  cat <<'EOF'
Usage:
  run_plan0_resume_standard_dr_promax.sh [--exp-dir PATH] [--ks LIST] [--seeds LIST]
...
LOG_FILE="$LOG_DIR/resume_plan0_standard_dr_promax_${TS}.log"

echo "[$(date -Iseconds)] Starting promax resume run"
...
echo "[$(date -Iseconds)] Finished promax resume run"
