...
important biological signals from rare malignant cell populations.

The script will execute three main experiments on the CITE-seq subset:
1. Pan-Patient (No CV): Uses a supervised 'all_filtered' method, which first
   filters for genes expressed in >1% of cells, but then "rescues" any
   genes with a malignant enrichment ratio > 50.
2. Per-Patient (No CV): Uses the same supervised 'all_filtered' method as Exp 1.
3. Per-Patient (No CV): Uses a relaxed weak DEG screen (adj pval < 0.1, |LFC| > 0.05)
   without a minimum gene count, allowing the data to determine the number
...
# The old sys.path manipulation is removed.
# With the package installed in editable mode (`pip install -e .`),
# Python can find `sc_classification` automatically.
...
def run_supervised_filtering_experiments(input_data_path: str, n_components: int = 100):
    """
    Sets up and runs a series of classification experiments with supervised filtering.
    """
    print("--- Starting Supervised Filtering Experiments for CITE-seq MRD ---")
    ...
    base_config_dict = {
        ...
        'classification': {
            'method': 'lr_lasso', 'alphas': 'logspace(-4, 5, 20)', 'cv_folds': 0, 
            'random_state': 42
        },
        'downsampling': {
    ...
    print("--- Running Exp 1: Pan-Patient, Supervised All Filtered ---")
    ...
    print("\n--- Running Exp 2: Per-Patient, Supervised All Filtered ---")
    ...
    print("\n--- Configuring Exp 3: Relaxed Weak DEG Screen ---")
...
def main():
    ...
    run_supervised_filtering_experiments(input_data_path=input_data_path)
