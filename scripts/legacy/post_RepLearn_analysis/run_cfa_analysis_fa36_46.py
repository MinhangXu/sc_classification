...
# --- Configuration for 2-Factor CFA ---
...
# A single, focused scenario for this analysis.
ANALYSIS_SCENARIOS = [
    {
        "name": "fa36_vs_fa46_pats_P02_P09_P13",
        "patient_subset": ['P02', 'P09', 'P13'],
...
def load_data_for_cfa(analyzer: ExperimentAnalyzer, 
    ...
    try:
        ...
        if patient_subset:
            print(f"\nSubsetting data to {len(patient_subset)} patients: {', '.join(patient_subset)}")
    ...
    return expression_df, filtered_gene_sets, adata.shape
...
def main():
    ...
    for i, scenario in enumerate(ANALYSIS_SCENARIOS):
        ...
        expression_df, leading_edge_gene_sets, raw_data_shape = load_data_for_cfa(
            analyzer, 
            exp_id, 
            all_factors_to_load, 
            PATHWAY_NAME,
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(f"Data Shape Before Gene Filtering: {raw_data_shape[0]} cells x {raw_data_shape[1]} genes\n")
